...
CSV はヘッダにある `CHnn_peak` を全部読み、`Shot.peaks` に可変長で持つ。画面に
出すのは `ShotHistory.used_channels()` が返す「一度でも非ゼロだった ch」だけ。
未接続の ch も本体は 0.00 で書き続けるので、列の有無では判別できない。
ch が 4 本以上になったらカードは自動で小型版に切り替わる。
